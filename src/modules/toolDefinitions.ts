...
function createSearchProperties() {
  return {
    text: {
      type: "string",
      description: "Simple keyword search for single terms or exact phrases (e.g., 'kubernetes', 'machine learning'). NEVER use for boolean logic like 'docker OR kubernetes' - this will search for the literal phrase 'docker OR kubernetes' instead of either term. For multiple term searches with OR/AND logic, use searchCriteria with type 'fulltext' instead: [{property: 'fulltext', type: 'fulltext', value: 'docker', logic: 'OR'}, {property: 'fulltext', type: 'fulltext', value: 'kubernetes'}].",
    },
    searchCriteria: {
      type: "array",
      ...
      items: {
        ...
        properties: {
          ...
          logic: {
            type: "string",
            enum: ["AND", "OR"],
            description: "Logic operator to combine with NEXT search criteria item. Enables cross-type boolean expressions like 'relation OR noteProperty' or 'hierarchy OR dateCreated'. Required for explicit readability. System automatically ignores logic on last item.",
            default: "AND"
          }
        },
