...
import { logVerboseInput, logVerboseOutput, logVerboseTransform } from "../../../modules/utils/system/verboseUtils.js";
import { translateTemplateNameToId, isBuiltinTemplate } from "../../../modules/utils/content/templateMapper.js";

// Unified SearchCriteria interface for all search types
interface SearchCriteria {
  property: string;  // Property name (varies by type)
  type: 'label' | 'relation' | 'noteProperty'; // Type of search criteria
  op?: string;       // Operator (exists, =, !=, >=, <=, >, <, contains, starts_with, ends_with, regex)
  value?: string;    // Value to search for (optional for exists)
...
function buildAttributeQuery(criteria: SearchCriteria): string {
  ...
  // Translate template values if this is a template relation with built-in template
  let processedValue = value;
  if (type === 'relation' && name === 'template' && value && isBuiltinTemplate(value)) {
    processedValue = translateTemplateNameToId(value);
    logVerboseTransform("searchQueryBuilder", value, processedValue, `Translated template name`);
  }
  ...
  switch (op) {
    ...
    case '=':
      ...
      return `${prefix}${escapedName} = '${processedValue.replace(/'/g, "\\'")}'`;
    case '!=':
      if (!processedValue) return '';
      return `${prefix}${escapedName} != '${processedValue.replace(/'/g, "\\'")}'`;
    case '>=':
...
function buildNotePropertyQuery(criteria: SearchCriteria): string {
  ...
  switch (op) {
    case 'exists':
      // For exists operator, just return the property name
      return triliumProperty;
    case 'not_exists':
      // For not_exists operator, return negated property
      return `!${triliumProperty}`;
    case '=':
      triliumOperator = '=';
      break;
