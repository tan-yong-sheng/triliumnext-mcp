...
export function createReadTools(): any[] {
  ...
  return [
    ...
    {
      name: "search_notes",
      description: "🔎 **SMART SEARCH with two distinct approaches**: Choose wisely based on what the user is looking for:\n\n⚠️ **CRITICAL: NEVER use 'noteId exists' for ambiguous 'list all notes' requests** - this returns thousands of notes and overwhelms users. ALWAYS ask: 'Which folder would you like me to list notes from?'\n\n📝 **Simple content discovery?** Use 'text' parameter for broad searches across both title AND content when users say general things like 'find notes containing docker' or 'search for kubernetes'.\n\n🎯 **Specific targeting or listing?** Use 'searchCriteria' when users specify WHERE to search ('in title', 'in content'), need complex logic ('docker OR kubernetes'), want to list notes in specific locations ('list notes under folder', 'show me notes under specific folder'), filter by properties ('notes created in 2024', 'archived notes'), hierarchy navigation ('show me notes under Projects folder', 'list all children of this note', 'find all notes beneath Meetings including subfolders'), note types ('find all code notes', 'show me book notes'), template searches ('find Board notes', 'Calendar notes'), or any criteria-based searches.\n\n✨ **POWERFUL CAPABILITIES**: Field-specific searches (title-only, content-only), date ranges, regex patterns, boolean logic, label/attribute filtering, template searches, hierarchy navigation (parents.title, ancestors.noteId), note type filtering, and MIME type searches.\n\n📋 **IMPORTANT USAGE GUIDANCE**:\n• Use 'text' for: 'find notes containing', 'search for', 'show me notes about'\n• Use 'searchCriteria' for: 'list notes under folder', 'notes created in', 'find all code notes', 'notes under Projects', 'show me notes under specific folder'\n• ⚠️ **NEVER use 'noteId exists' for 'list all notes' requests** - this returns EVERY note in your entire database (thousands of notes) and is never what users want. ALWAYS ask for clarification: 'Which folder would you like me to list notes from?' or 'Are you looking for notes under a specific folder?'\n• For complex boolean logic: always use searchCriteria with AND/OR operators\n\n🗺️ **HIERARCHY & FOLDER NAVIGATION** (like exploring folders):\n• Direct children only: 'show me notes under Projects', 'list all notes in the Meetings folder', 'what\\'s inside the Projects folder?'\n• All descendants including subfolders: 'show me everything under Projects including subfolders', 'find all notes beneath Meetings', 'list all notes in the hierarchy under Projects'\n• Parent navigation: 'find the parent folder of this note', 'show me notes that have this note as a child', 'what notes contain this note as a subfolder?'",
      inputSchema: {
        type: "object",
...
function createSearchProperties() {
  return {
    ...
    searchCriteria: {
      type: "array",
      description: "Precise field-specific searches. REQUIRED for: 'list notes under folder', 'notes created in 2024', 'find all code notes', 'archived notes', 'show me notes under Projects folder', 'list all children of this note', 'find all notes beneath Meetings including subfolders', 'Board template notes', 'Calendar notes'. ⚠️ **NEVER use with 'noteId exists' for any 'list all notes' requests** - this returns thousands of notes and is always the wrong approach. Supports operators: 'contains', 'starts_with', 'ends_with', '=', '!=', '>', '<', '>=', '<=', 'exists', 'regex'. Boolean logic: 'AND'/'OR'. Search properties: 'title', 'content', 'type', 'mime', 'dateCreated', 'dateModified', 'isArchived', 'parents.title', 'ancestors.title', 'template.title'. Labels: '#important', '#project', '#status'. Relations: '~author', '~template'. Hierarchy navigation: 'parents.title' (direct children), 'ancestors.title' (all descendants including subfolders). Note types: 'text', 'code', 'mermaid', 'book'. MIME types: 'application/typescript', 'text/x-python'. Templates: 'Board', 'Calendar', 'Text Snippet'.",
      items: {
        type: "object",
        properties: {
          property: {
