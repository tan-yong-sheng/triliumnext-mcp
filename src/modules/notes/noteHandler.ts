...
} from "../../modules/utils/recovery/noteRecovery.js";
...
export async function handleUpdateNoteRequest(
  ...
  try {
    ...
    // Check if this is a 404 error (note not found)
    if ((error as any).response?.status === 404 || (error as Error)?.message?.includes('not found')) {
      // Extract potential note name from the failed noteId
      const potentialName = extractNoteNameFromId(args.noteId);

      // Search for similar notes
      const searchResults = await searchForSimilarNotes(potentialName, axiosInstance);

      // Generate helpful recovery message with update context
